...
* File:          RightFollow.c
* Date:          12/2/12
* Description:   Follows the Right Wall 
* Author:        Brennan McTernan        
* Modifications:  1
*/
...
typedef enum { ST_UNDEFINED, ST_OK, ST_TOO_FAR, ST_TOO_CLOSE, ST_LEFT_TURN, ST_RIGHT_TURN_START, ST_RIGHT_TURN, ST_NUM_STATES } tState  ;
char* gStateNames [] = {"ST_UNDEFINED", "ST_OK", "ST_TOO_FAR", "ST_TOO_CLOSE", "ST_LEFT_TURN", "ST_RIGHT_TURN_START", "ST_RIGHT_TURN", "ST_NUM_STATES" } ;
...
#define ST_TOO_FAR_SPD_L 100
#define ST_TOO_FAR_SPD_R 20

#define ST_TOO_CLOSE_SPD_L 20
#define ST_TOO_CLOSE_SPD_R 100

#define ST_RIGHT_TURN_START_SPD_L 100
#define ST_RIGHT_TURN_START_SPD_R 100

#define ST_RIGHT_TURN_SPD_L 100
#define ST_RIGHT_TURN_SPD_R 40

#define ST_LEFT_TURN_SPD_L -100
        ...
        */
        ...
        switch(state)
        {
            case ST_OK :
                ...
                else if(ir_values[SENSOR_RDIAG] > THRESHOLD_CLOSE)
                {
                    printf ("Too close to right wall\n") ;
                    //Drifting towards the wall
                    state = ST_TOO_CLOSE ;
                }
                // Am I too far from the right wall?
                else if(ir_values[SENSOR_RDIAG] < THRESHOLD_FAR)
                {
                    if (ir_values[SENSOR_RDIAG] < THRESHOLD_NO_WALL)
                    {
                      printf ("LOST right wall\n") ;
                      //Drifting away from wall
                      state = ST_RIGHT_TURN_START ;
                    }
                    else
                    {
                ...
                break ;
            case ST_TOO_FAR :
                //Too far from right wall, turn back towards wall.
                
                // turn to the right...
                speed (file, ST_TOO_FAR_SPD_L, ST_TOO_FAR_SPD_R) ;
                // Did I turn enough?
                if (ir_values[SENSOR_RDIAG] > THRESHOLD_FAR_OK)  
                    state = ST_OK ; // Yes- go back to "OK"
                else if (ir_values[SENSOR_RDIAG] < THRESHOLD_NO_WALL)  
                    state = ST_RIGHT_TURN_START ; // Yes- go back to "OK"
                    
                break ;
            case ST_TOO_CLOSE :
                // Too close to right wall, turn away from wall
                
                // turn to the left...
                speed (file, ST_TOO_CLOSE_SPD_L, ST_TOO_CLOSE_SPD_R) ;
                // Did I turn enough?
                if (ir_values[SENSOR_RDIAG] < THRESHOLD_CLOSE)  
                    state = ST_OK ; // Yes- go back to "OK"
                
                break ;
            case ST_RIGHT_TURN_START :
                //lost right wall, Make sweeping right turn.
                
                // turn to the right...
                speed (file, ST_RIGHT_TURN_START_SPD_L, ST_RIGHT_TURN_START_SPD_R) ;
                // Did I turn enough?
                if (ir_values[SENSOR_RIGHT] < THRESHOLD_NO_WALL)  
                    state = ST_RIGHT_TURN ; // Yes- go back to "OK"
                break;
            case ST_RIGHT_TURN :
                //lost right wall, Make sweeping left turn.
                
                // turn to the right...
                speed (file, ST_RIGHT_TURN_SPD_L, ST_RIGHT_TURN_SPD_R) ;
                // Did I turn enough?
                if (ir_values[SENSOR_RDIAG] > THRESHOLD_FAR)  
                    state = ST_OK ; // Yes- go back to "OK"
                break;
            case ST_LEFT_TURN :
                //Something wrong, spin left
                
                // turn to the left...
                speed (file, ST_LEFT_TURN_SPD_L, ST_LEFT_TURN_SPD_R) ;
                // Did I turn enough?
                if ((ir_values[SENSOR_RFRONT] < THRESHOLD_NO_WALL) && (ir_values[SENSOR_LFRONT] < THRESHOLD_NO_WALL)) 
                    state = ST_OK ; // Yes- go back to "OK"
                break;
            default :
